...
def list_windows(window_title: str = "") -> list[tuple[int, str]]:
...
def send_key(hwnd: int, vk_code: int, mode: str = "foreground") -> None:
    """发送按键。
    ...
    """
    if mode == "postmsg":
        send_key_postmsg(hwnd, vk_code)
    elif mode == "focus":
        send_key_focus(hwnd, vk_code)
    else:
        send_key_foreground(hwnd, vk_code)


def send_key_foreground(hwnd: int, vk_code: int) -> None:
    """前台模式：激活游戏窗口后用 pydirectinput 发送。"""
    set_foreground(hwnd)
    _press_vk(vk_code)


def send_key_focus(hwnd: int, vk_code: int) -> None:
    """快速切焦点模式：保存当前前台→激活游戏→发键→切回。

    每次约 300ms 打断用户操作，适合低频率按键。
    """
...
def send_key_postmsg(hwnd: int, vk_code: int) -> None:
    """后台模式：用 PostMessage 发送 WM_KEYDOWN/WM_KEYUP。

    不需要窗口在前台，不打扰用户。
    但 DirectInput 游戏可能不响应此消息。
    """
    scan = _VK_TO_SCAN.get(vk_code, vk_code)
    # lParam: bits 16-23 = scancode, bit 30 = previous state (0), bit 31 = transition (0=down)
    lparam_down = (scan << 16) | 1  # repeat count = 1
    lparam_up = (scan << 16) | 0xC0000001  # transition + previous state
...
def send_key_down(hwnd: int, vk_code: int, mode: str = "foreground") -> None:
    """按键按下（不松开，用于按住场景）。

    Args:
        hwnd: 目标窗口句柄。
        vk_code: 虚拟键码。
        mode: 输入模式，同 send_key。
    """
    if mode == "postmsg":
        scan = _VK_TO_SCAN.get(vk_code, vk_code)
        # 方向键等扩展键需要 E0 前缀
        is_extended = vk_code in (0x25, 0x26, 0x27, 0x28)
...
def send_key_up(hwnd: int, vk_code: int, mode: str = "foreground") -> None:
    """按键松开。
...
if __name__ == "__main__":
    ...
    title = input("请输入窗口标题（如 QQ三国）: ").strip() or "QQ三国"
    hwnd = get_hwnd(title)
    if not hwnd:
        print("未找到窗口，请确认窗口标题")
        raise SystemExit(1)
    ...
    print(f"窗口句柄: {hwnd}")
    print(f"是否最小化: {is_minimized(hwnd)}")

    # 测试按键
    ok = set_foreground(hwnd)
    print(f"窗口激活: {'成功' if ok else '失败'}")
    if ok:
        print("3 秒后发送 '1' 键...")
        time.sleep(3)
